...
#include <pcms/field/evaluator/mls_interpolation_impl.hpp>
...
struct RBF_CONST
{
  ...
  double operator()(double r_sq, double rho_sq) const
  {
    OMEGA_H_CHECK_PRINTF(
      rho_sq > 0, "ERROR: rho_sq in rbf has to be positive, but got %.16f\n",
      rho_sq);
    double phi = (rho_sq < r_sq) ? 0.0 : 1.0;
    OMEGA_H_CHECK_PRINTF(!std::isnan(phi),
                         "ERROR: phi in rbf is NaN. r_sq, rho_sq = (%f, %f)\n",
                         r_sq, rho_sq);
    return phi;
...
struct RBF_MULTIQUADRIC
{
  ...
  double operator()(double r_sq, double rho_sq) const
  {
    ...
    double ratio = r / rho;
    double phi =
      (rho_sq < r_sq) ? 0.0 : Kokkos::sqrt(1.0 + a * a * ratio * ratio);
    OMEGA_H_CHECK_PRINTF(!std::isnan(phi),
                         "ERROR: phi in rbf is NaN. r_sq, rho_sq = (%f, %f)\n",
                         r_sq, rho_sq);
    return phi;
...
struct RBF_INVMULTIQUADRIC
{
  ...
  double operator()(double r_sq, double rho_sq) const
  {
    ...
    double ratio = r / rho;
    double phi =
      (rho_sq < r_sq) ? 0.0 : 1.0 / Kokkos::sqrt(1.0 + a * a * ratio * ratio);
    OMEGA_H_CHECK_PRINTF(!std::isnan(phi),
                         "ERROR: phi in rbf is NaN. r_sq, rho_sq = (%f, %f)\n",
                         r_sq, rho_sq);
    return phi;
...
struct RBF_THINPLATESPLINE
{
  ...
  double operator()(double r_sq, double rho_sq) const
  {
    ...
    double ratio = r / rho;
    double phi =
      (rho_sq < r_sq) ? 0.0 : a * a * ratio * ratio * Kokkos::log(a * ratio);
    OMEGA_H_CHECK_PRINTF(!std::isnan(phi),
                         "ERROR: phi in rbf is NaN. r_sq, rho_sq = (%f, %f)\n",
                         r_sq, rho_sq);
    return phi;
...
struct RBF_CUBIC
{
  ...
  double operator()(double r_sq, double rho_sq) const
  {
    ...
    double rho = Kokkos::sqrt(rho_sq);
    double ratio = r / rho;
    double phi = (rho_sq < r_sq) ? 0.0 : a * a * a * ratio * ratio * ratio;
    OMEGA_H_CHECK_PRINTF(!std::isnan(phi),
                         "ERROR: phi in rbf is NaN. r_sq, rho_sq = (%f, %f)\n",
                         r_sq, rho_sq);
    return phi;
  ...
  double lambda, double tol, double decay_factor)
{
  ...
  switch (bf) {
    ...
    case RadialBasisFunction::NO_OP:
      ...
      break;
    case RadialBasisFunction::RBF_MULTIQUADRIC:
      interpolated_values = detail::mls_interpolation(
        source_values, source_coordinates, target_coordinates, support, dim,
        degree, RBF_MULTIQUADRIC{decay_factor}, lambda, tol);
      break;
    case RadialBasisFunction::RBF_INVMULTIQUADRIC:
      interpolated_values = detail::mls_interpolation(
        source_values, source_coordinates, target_coordinates, support, dim,
        degree, RBF_INVMULTIQUADRIC{decay_factor}, lambda, tol);
      break;
    case RadialBasisFunction::RBF_THINPLATESPLINE:
      interpolated_values = detail::mls_interpolation(
        source_values, source_coordinates, target_coordinates, support, dim,
        degree, RBF_THINPLATESPLINE{decay_factor}, lambda, tol);
      break;
    case RadialBasisFunction::RBF_CUBIC:
      interpolated_values = detail::mls_interpolation(
        source_values, source_coordinates, target_coordinates, support, dim,
        degree, RBF_CUBIC{decay_factor}, lambda, tol);
      break;
  }
