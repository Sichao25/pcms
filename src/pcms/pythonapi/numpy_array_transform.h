...
// Helper function to convert Rank1 View to array (creates a reference, zero-copy)
// Note: Caller must use pybind11::keep_alive to manage the source object's lifetime
template<typename T>
...
// Overload: Helper function to convert Rank2 View to array (creates a reference, zero-copy)
// Note: Caller must use pybind11::keep_alive to manage the source object's lifetime
template<typename T>
py::array_t<typename std::remove_const<T>::type> view_to_numpy(Rank2View<T, HostMemorySpace> view) {
  using NonConstT = typename std::remove_const<T>::type;
  return py::array_t<NonConstT>(
    {static_cast<py::ssize_t>(view.extent(0)), static_cast<py::ssize_t>(view.extent(1))},  // shape
    {sizeof(NonConstT) * view.extent(1), sizeof(NonConstT)},                                 // strides (row-major)
    const_cast<NonConstT*>(view.data_handle())                                               // data pointer
    // No base object - lifetime managed by keep_alive<> policy at binding site
  );
}
...
// Overload: Helper to convert 2D Kokkos View to numpy array (creates a reference)
template<typename T>
py::array_t<T> kokkos_view_to_numpy(Kokkos::View<T**, HostMemorySpace> view) {
  return py::array_t<T>(
    {static_cast<py::ssize_t>(view.extent(0)), static_cast<py::ssize_t>(view.extent(1))},  // shape
...
// Overload: Helper to convert 3D Kokkos View to numpy array (creates a reference)
template<typename T>
py::array_t<T> kokkos_view_to_numpy(Kokkos::View<T***, HostMemorySpace> view) {
  return py::array_t<T>(
...
// Helper function to convert mdspan View to array (creates a reference, zero-copy)
// Note: Caller must use pybind11::keep_alive to manage the source object's lifetime
template<typename T>
