...
void bind_mls_interpolation_module(py::module& m) {
  py::class_<SupportResults>(m, "SupportResults")
    ...
    .def(py::init([](py::array_t<Omega_h::LO> supports_ptr,
      ...
      auto supports_ptr_read = numpy_to_omega_h_read<Omega_h::LO>(supports_ptr);
      auto supports_idx_read = numpy_to_omega_h_read<Omega_h::LO>(supports_idx);
      auto radii2_write = numpy_to_omega_h_write<Omega_h::Real>(radii2);
      return SupportResults{supports_ptr_read, supports_idx_read, radii2_write};
    }),
    py::arg("supports_ptr"),
    py::arg("supports_idx"),
    py::arg("radii2"))
  ...
  m.def(
    ...
    [](py::array_t<Omega_h::Real> source_values,
       ...
       Omega_h::LO dim,
       Omega_h::LO degree,
       pcms::RadialBasisFunction bf,
       double lambda_reg,
       double tol,
       double decay_factor) {
      auto source_values_read = numpy_to_omega_h_read<Omega_h::Real>(source_values);
      auto source_coordinates_read = numpy_to_omega_h_read<Omega_h::Real>(source_coordinates);
      auto target_coordinates_read = numpy_to_omega_h_read<Omega_h::Real>(target_coordinates);
    ...
    py::arg("source_coordinates"),
    py::arg("target_coordinates"),
    py::arg("support"),
    py::arg("dim"),
    py::arg("degree"),
