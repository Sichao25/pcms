...
template<typename T>
void flatten_vector4d(const std::vector<std::vector<std::vector<std::vector<T>>>>& vec4d, std::vector<T>& vec) {
    std::size_t idx = 0;
...
void vector4d_to_array(const std::vector<std::vector<std::vector<std::vector<T>>>>& vec4d, std::array<T, N>& arr) {
    std::size_t idx = 0;
    for (const auto& cube : vec4d) {
        for (const auto& plane : cube) {
            for (const auto& row : plane) {
...
void tset(int nth, std::vector<double>& th, std::vector<double>& sth, std::vector<double>& cth, double thmin, double thmax) {
    for (int ith = 0; ith < nth; ++ith) {
        th[ith] = thmin + static_cast<double>(ith) * (thmax - thmin) / static_cast<double>(nth - 1);
...
void xset(int nx, std::vector<double>& x, std::vector<double>& ex,
          double xmin, double xmax) {
    if (nx < 2) return; // avoid division by zero

    for (int ix = 0; ix < nx; ++ix) {
        x[ix] = xmin + static_cast<double>(ix) * (xmax - xmin) / static_cast<double>(nx - 1);
        ex[ix] = std::exp(2.0 * x[ix] - 1.0);
    }
}

// void ffset(int num, const std::vector<double>& xf, const std::vector<double>& tf,
...
void bset(Rank1View<double, HostMemorySpace> fx, int nx,
          ...
          Rank1View<double, HostMemorySpace> bcx1, Rank1View<double, HostMemorySpace> bcx2,
          Rank1View<double, HostMemorySpace> bcth1, Rank1View<double, HostMemorySpace> bcth2) {
...
void dotest1(int ns, Rank1View<const double, HostMemorySpace> x, Rank1View<double, HostMemorySpace> f,
    Rank1View<double, HostMemorySpace> fd, Rank2View<double, HostMemorySpace> fspl, Rank2View<double, HostMemorySpace> fspp,
    Rank2View<double, HostMemorySpace> fs2, int nt, Rank1View<const double, HostMemorySpace> xt,
    Rank1View<double, HostMemorySpace> ft, Rank2View<double, HostMemorySpace> xpkg, Rank2View<double, HostMemorySpace> testa1,
    Rank2View<double, HostMemorySpace> testa2, Rank2View<double, HostMemorySpace> testa3, Rank1View<double, HostMemorySpace> wk,
...
void pspltest1(double zctrl) {
    ...
    // Local arrays
    std::vector<double> zdum_vec(1000), testa1_vec(1000), testa2_vec(1000), testa3_vec(1000), xtest_vec(1000), ftest_vec(1000);
    ...
    auto zdum = Rank1View<double, HostMemorySpace>(zdum_vec.data(), zdum_vec.size());
    std::vector<double> wk2_vec(1000);
    std::copy(zdum_vec.begin(), zdum_vec.end(), wk2_vec.begin());
    auto wk2 = Rank1View<double, HostMemorySpace>(wk2_vec.data(),wk2_vec.size());
    // std::array<double, 3000> testa1_arr;
    // to_array(testa1_vec, testa1_arr);
    auto testa1 = Rank2View<double, HostMemorySpace>(testa1_vec.data(), 1000, 3);
    // std::array<double, 3000> testa2_arr;
    // to_array(testa2_vec, testa2_arr);
    auto testa2 = Rank2View<double, HostMemorySpace>(testa2_vec.data(), 1000, 3);
    // std::array<double, 3000> testa3_arr;
    // to_array(testa3_vec, testa3_arr);
    auto testa3 = Rank2View<double, HostMemorySpace>(testa3_vec.data(), 1000, 3);
    // std::array<double, 10> x_arr;
    // to_array(x_vec, x_arr);
    auto x = Rank1View<const double, HostMemorySpace>(x_vec.data(), x_vec.size());
    // std::array<double, 10> zcos_arr;
    // to_array(zcos_vec, zcos_arr);
    auto zcos = Rank1View<double, HostMemorySpace>(zcos_vec.data(), zcos_vec.size());
    // std::array<double, 10> z2sin_arr;
    // to_array(z2sin_vec, z2sin_arr);
    auto z2sin = Rank1View<double, HostMemorySpace>(z2sin_vec.data(), z2sin_vec.size());
    // std::array<double, 40> fs_arr;
    // to_array(fs_vec, fs_arr);
    auto fs = Rank2View<double, HostMemorySpace>(fs_vec.data(), 4, 10);
    // std::array<double, 40> fsp_arr;
    // to_array(fsp_vec, fsp_arr);
    auto fsp = Rank2View<double, HostMemorySpace>(fsp_vec.data(), 4, 10);
    // std::array<double, 20> fs2_arr;
    // to_array(fs2_vec, fs2_arr);
    auto fs2 = Rank2View<double, HostMemorySpace>(fs2_vec.data(), 2, 10);
    // std::array<double, 40> xpkg_arr;
    // to_array(xpkg_vec, xpkg_arr);
    auto xpkg = Rank2View<double, HostMemorySpace>(xpkg_vec.data(), 10, 4);
    ...
    // Call test function
    dotest1(inum, x, z2sin, zcos, fs, fsp, fs2, 1000, xtest, ftest, xpkg, testa1, testa2, testa3, zdum, wk2);
}
...
void compare(const std::string& slbl,
             ...
             Rank2View<double, HostMemorySpace> fl,
             int ilinx, int ilinth,
    ...
    int icycle = 20;
    int iherm = 0;
    if (slbl == "hermite") iherm = 1;
    else if (slbl == "mkbicub") iherm = 2;
    else if (slbl == "piecewise linear") iherm = 3;
    ...
    for (int j = 0; j < ntest; ++j) {
        ...
        for (int i = 0; i < ntest; ++i) {
            ...
            if (iherm == 0) {
                BiCubicSplineInterpolator<double, HostMemorySpace>::bcspeval(zx,zth,
                    isel,fget,x,nx,th,nth,ilinx,ilinth,
                    f,nx,ier);
            } else if (iherm == 2) {
                BiCubicSplineInterpolator<double, HostMemorySpace>::evbicub(zx,zth,
...
void dotest2(Rank1View<const double, HostMemorySpace> x,Rank1View<double, HostMemorySpace> fx, int nx,
    Rank1View<const double, HostMemorySpace> th,Rank1View<double, HostMemorySpace>fth,
    Rank1View<double, HostMemorySpace> dfth, int nth,
    Rank4View<double, HostMemorySpace> f,Rank3View<double, HostMemorySpace> fh,
    Rank2View<double, HostMemorySpace>flin,
    Rank1View<double, HostMemorySpace> bcx1, Rank1View<double, HostMemorySpace> bcx2,
    Rank1View<double, HostMemorySpace> bcth1, Rank1View<double, HostMemorySpace> bcth2,
    Rank1View<double, HostMemorySpace> xtest, Rank1View<double, HostMemorySpace> fxtest,
    Rank1View<double, HostMemorySpace> thtest, Rank1View<double, HostMemorySpace> fthtest,
    int ntest) {
    for (int ith = 0; ith < nth; ++ith) {
        for (int ix = 0; ix < nx; ++ix) {
    ...
    interpolator.mkbicub(x, nx, th, nth, fh, nx,
            nbc, bcx1, nbc, bcx2,
            nbc, bcth1, nbc, bcth2,
            ilinx, ilinth, ier);
...
void pspltest2(double zctrl) {
    ...
    std::vector<double> x1_vec(10), ex1_vec(10), t1_vec(10), st1_vec(10), ct1_vec(10);
    std::vector<double> x2_vec(20), ex2_vec(20), t2_vec(20), st2_vec(20), ct2_vec(20);
    std::vector<double> x4_vec(40), ex4_vec(40), t4_vec(40), st4_vec(40), ct4_vec(40);
    std::vector<double> xtest_vec(200), extest_vec(200), ttest_vec(200), stest_vec(200), ctest_vec(200);
    // std::vector<std::vector<std::vector<std::vector<double>>>> f1_vec(4, std::vector<std::vector<std::vector<double>>>(4, std::vector<std::vector<double>>(10, std::vector<double>(10))));
    // std::vector<std::vector<std::vector<std::vector<double>>>> f2_vec(4, std::vector<std::vector<std::vector<double>>>(4, std::vector<std::vector<double>>(20, std::vector<double>(20))));
    // std::vector<std::vector<std::vector<std::vector<double>>>> f4_vec(4, std::vector<std::vector<std::vector<double>>>(4, std::vector<std::vector<double>>(40, std::vector<double>(40))));
    ...
    auto x1 = Rank1View<const double, HostMemorySpace>(x1_vec.data(), x1_vec.size());

    // std::array<double, 10> ex1_arr;
    // to_array(ex1_vec, ex1_arr);
    auto ex1 = Rank1View<double, HostMemorySpace>(ex1_vec.data(), ex1_vec.size());

    // std::array<double, 10> t1_arr;
    // to_array(t1_vec, t1_arr);
    auto t1 = Rank1View<const double, HostMemorySpace>(t1_vec.data(), t1_vec.size());

    // std::array<double, 10> st1_arr;
    // to_array(st1_vec, st1_arr);
    auto st1 = Rank1View<double, HostMemorySpace>(st1_vec.data(), st1_vec.size());
    // std::array<double, 10> ct1_arr;
    // to_array(ct1_vec, ct1_arr);
    auto ct1 = Rank1View<double, HostMemorySpace>(ct1_vec.data(), ct1_vec.size());

    // // std::array<double, 1600> f1_arr;
    // // vector4d_to_array(f1_vec, f1_arr);
    // std::vector<double> f1_vec_flattened(1600);
    // flatten_vector4d(f1_vec, f1_vec_flattened);
    auto f1 = Rank4View<double, HostMemorySpace>(f1_vec.data(), Kokkos::extents{4, 4, 10, 10});
    // std::array<double, 1600> flin_arr;
    // to_array(flin_vec, flin_arr);
    auto flin = Rank2View<double, HostMemorySpace>(flin_vec.data(), 10, 10);
    // std::array<double, 6400> fh_arr;
    // to_array(fh_vec, fh_arr);
    auto fh = Rank3View<double, HostMemorySpace>(fh_vec.data(), 4, 10, 10);

    // std::array<double, 40> bcx1_arr;
    // to_array(bcx1_vec, bcx1_arr);
    auto bcx1 = Rank1View<double, HostMemorySpace>(bcx1_vec.data(), bcx1_vec.size());
    // std::array<double, 40> bcx2_arr;
    // to_array(bcx2_vec, bcx2_arr);
    auto bcx2 = Rank1View<double, HostMemorySpace>(bcx2_vec.data(), bcx2_vec.size());
    // std::array<double, 40> bcth1_arr;
    // to_array(bcth1_vec, bcth1_arr);
    auto bcth1 = Rank1View<double, HostMemorySpace>(bcth1_vec.data(), bcth1_vec.size());
    // std::array<double, 40> bcth2_arr;
    // to_array(bcth2_vec, bcth2_arr);
    auto bcth2 = Rank1View<double, HostMemorySpace>(bcth2_vec.data(), bcth2_vec.size());

    // std::array<double, 200> xtest_arr;
    // to_array(xtest_vec, xtest_arr);
    auto xtest = Rank1View<double, HostMemorySpace>(xtest_vec.data(), xtest_vec.size());
    // std::array<double, 200> extest_arr;
    // to_array(extest_vec, extest_arr);
    auto extest = Rank1View<double, HostMemorySpace>(extest_vec.data(), extest_vec.size());
    // std::array<double, 200> ttest_arr;
    // to_array(ttest_vec, ttest_arr);
    auto ttest = Rank1View<double, HostMemorySpace>(ttest_vec.data(), ttest_vec.size());
    // std::array<double, 200> stest_arr;
    // to_array(stest_vec, stest_arr);
    auto stest = Rank1View<double, HostMemorySpace>(stest_vec.data(), stest_vec.size());
    // std::array<double, 200> ctest_arr;
    // to_array(ctest_vec, ctest_arr);
    // auto ctest = Rank1View<double, HostMemorySpace>(ctest_arr.data(), ctest_arr.size());

    dotest2(x1,ex1,10,t1,st1,ct1,10,f1,fh,flin,
