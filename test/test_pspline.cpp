...
void pspltest1(double zctrl) {
    ...
    auto zdum = Rank1View<double, HostMemorySpace>(zdum_arr.data(), zdum_arr.size());
    ...
    auto zcos = Rank1View<double, HostMemorySpace>(zcos_arr.data(), zcos_arr.size());
    std::array<double, 10> z2sin_arr;
    to_array(z2sin_vec, z2sin_arr);
    auto z2sin = Rank1View<double, HostMemorySpace>(z2sin_arr.data(), z2sin_arr.size());
...
void compare(const std::string& slbl,
             ...
             int ntest, BiCubicSplineInterpolator<double, HostMemorySpace> interpolator) {
...
void dotest2(Rank1View<const double, HostMemorySpace> x,Rank1View<double, HostMemorySpace> fx, int nx,
    ...
    compare("bcspline", x, nx, th, nth, f, fh, flin, ilinx, ilinth,
            xtest, fxtest, thtest, fthtest, ntest, interpolator);
...
void pspltest2(double zctrl) {
    ...
    auto x1 = Rank1View<const double, HostMemorySpace>(x1_arr.data(), x1_arr.size());
    ...
    auto t1 = Rank1View<const double, HostMemorySpace>(t1_arr.data(), t1_arr.size());
    ...
    auto f1 = Rank4View<double, HostMemorySpace>(f1_arr.data(), Kokkos::extents{4, 4, 10, 10});
    ...
    auto bcx1 = Rank1View<double, HostMemorySpace>(bcx1_arr.data(), bcx1_arr.size());
    std::array<double, 40> bcx2_arr;
    to_array(bcx2_vec, bcx2_arr);
    auto bcx2 = Rank1View<double, HostMemorySpace>(bcx2_arr.data(), bcx2_arr.size());
    std::array<double, 40> bcth1_arr;
    to_array(bcth1_vec, bcth1_arr);
    auto bcth1 = Rank1View<double, HostMemorySpace>(bcth1_arr.data(), bcth1_arr.size());
    std::array<double, 40> bcth2_arr;
    to_array(bcth2_vec, bcth2_arr);
    auto bcth2 = Rank1View<double, HostMemorySpace>(bcth2_arr.data(), bcth2_arr.size());

    std::array<double, 200> xtest_arr;
    to_array(xtest_vec, xtest_arr);
    auto xtest = Rank1View<double, HostMemorySpace>(xtest_arr.data(), xtest_arr.size());
    std::array<double, 200> extest_arr;
    to_array(extest_vec, extest_arr);
    auto extest = Rank1View<double, HostMemorySpace>(extest_arr.data(), extest_arr.size());
    std::array<double, 200> ttest_arr;
    to_array(ttest_vec, ttest_arr);
    auto ttest = Rank1View<double, HostMemorySpace>(ttest_arr.data(), ttest_arr.size());
    std::array<double, 200> stest_arr;
    to_array(stest_vec, stest_arr);
    auto stest = Rank1View<double, HostMemorySpace>(stest_arr.data(), stest_arr.size());
...
int main(int argc, char** argv) {
    ...
    std::cout << "Total time for spline tests: " << time << " seconds" << std::endl;
    Kokkos::finalize();
    MPI_Finalize();
}