...
auto setupComms(redev::Redev& rdv, std::string clientName, const int clientId)
{
  const bool isSST = false;
  adios2::Params params{{"Streaming", "On"}, {"OpenTimeoutSecs", "60"}};
  REDEV_ALWAYS_ASSERT(clientId == 0 || clientId == 1);
  return rdv.CreateAdiosChannel(clientName, params,
                                static_cast<redev::TransportType>(isSST));
}
...
void client(Omega_h::Mesh& mesh, std::string fieldName, const int clientId)
{
  ...
  auto rdv =
    redev::Redev(MPI_COMM_WORLD, redev::Partition{std::move(partition)},
                 static_cast<redev::ProcessType>(isRdv));
  std::stringstream clientName;
  clientName << fieldName << "Client" << clientId;
  auto channel = setupComms(rdv, clientName.str(), clientId);
  auto comm = channel.CreateComm<redev::GO>(clientName.str(), rdv.GetMPIComm());

  auto isOverlap_h = markMeshOverlapRegion(mesh);
  //////////////////////////////////////////////////////
  // the non-rendezvous app sends global vtx ids to rendezvous
  //////////////////////////////////////////////////////
  ts::OutMsg appOut = ts::prepareAppOutMessage(
  ...
  comm.SetOutMessageLayout(
    appOut.dest,
    appOut.offset); // TODO - can this be moved to the AdiosComm ctor
  // fill message array
  auto gids = mesh.globals(0);
  auto gids_h = Omega_h::HostRead(gids);
...
void serverReceiveFromClient(ClientMetaData& clientMeta,
                             ...
                             redev::BidirectionalComm<redev::GO>& comm,
                             Omega_h::Mesh& mesh, std::string_view fieldName,
                             const int rank, const int clientId)
{
  std::stringstream ss;
  ...
  //  message directly, so the global ids may need to be sent during an
...
void serverSendToClient(ClientMetaData& clientMeta, redev::Channel& channel,
                        redev::BidirectionalComm<redev::GO>& comm,
                        Omega_h::Mesh& mesh,
                        Omega_h::HostRead<Omega_h::I8>& isOverlap_h,
                        std::string_view fieldName, const int rank,
                        const int clientId)
{
  // fill message array
  auto gids = mesh.globals(0);
  ...
  for (int i = 0; i < gids_h.size(); i++) {
    for (int j = clientMeta.outPermute.off[i];
         j < clientMeta.outPermute.off[i + 1]; j++) {
...
void server(Omega_h::Mesh& mesh, std::string fieldName,
            std::string_view cpnFileName)
{
  ...
  auto rdv = redev::Redev(MPI_COMM_WORLD, std::move(partition),
                          static_cast<redev::ProcessType>(isRdv));

  std::stringstream clientName0;
  clientName0 << fieldName << "Client0";
  std::stringstream clientName1;
  clientName1 << fieldName << "Client1";
  auto channel0 = setupComms(rdv, clientName0.str(), 0);
  auto channel1 = setupComms(rdv, clientName1.str(), 1);

  auto commClient0 =
    channel0.CreateComm<redev::GO>(clientName0.str(), rdv.GetMPIComm());
  auto commClient1 =
    channel1.CreateComm<redev::GO>(clientName1.str(), rdv.GetMPIComm());
  ...
  serverReceiveFromClient(client0, channel0, commClient0, mesh, fieldName, rank,
                          0);
  serverReceiveFromClient(client1, channel1, commClient1, mesh, fieldName, rank,
                          1);
  ts::writeVtk(mesh, "rdvInGids", 0);

  serverSendToClient(client0, channel0, commClient0, mesh, isOverlap_h,
                     fieldName, rank, 0);
  serverSendToClient(client1, channel1, commClient1, mesh, isOverlap_h,
                     fieldName, rank, 1);
  //////////////////////////////////////////////////////
  // communication loop
  //////////////////////////////////////////////////////
