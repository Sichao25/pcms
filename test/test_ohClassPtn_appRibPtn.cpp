...
int main(int argc, char** argv)
{
  try {
    ...
    if (argc != 3) {
      ...
      std::cerr
        << "mesh for the rendezvous processes and "
    ...
    for (int iter = 0; iter < 3; iter++) {
      ...
      if (!isRdv) {
        ...
        if (iter == 0) {
          ...
          //- set the message layout for the reverse
          //(rendezvous->non-rendezvous)
          // send by
      ...
      if (isRdv) {
        ...
        for (int i = 0; i < gids_h.size(); i++) {
          for (int j = rdvOutPermute.off[i]; j < rdvOutPermute.off[i + 1];
               j++) {
        ...
        { // check incoming messages are in the correct order
          auto gids = mesh.globals(0);
          auto gids_h = Omega_h::HostRead(gids);
          REDEV_ALWAYS_ASSERT(msgs.size() ==
                              static_cast<size_t>(gids_h.size()));
          for (size_t i = 0; i < msgs.size(); i++) {
