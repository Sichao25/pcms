...
int main(int argc, char** argv)
{
  ...
  if (isRdv) {
    ...
    auto rdv = redev::Redev(MPI_COMM_WORLD, redev::Partition{partition},
                            static_cast<redev::ProcessType>(isRdv));
    adios2::Params params{{"Streaming", "On"}, {"OpenTimeoutSecs", "60"}};
    auto channel = rdv.CreateAdiosChannel(std::string(name), params,
                                          redev::TransportType::BP4);
    auto comm =
      channel.CreateComm<redev::GO>(std::string(name), rdv.GetMPIComm());
    auto isOverlap_h = markMeshOverlapRegion(mesh);
    ts::OutMsg appOut = ts::OutMsg(); // FIXME is this needed?
    // TODO - Document why rendezvous needs two permutations but the app does
    // not
