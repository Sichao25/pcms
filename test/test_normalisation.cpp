...
TEST_CASE("test_normalization_routine")
{
  ...
  Omega_h::LOs nsupports_offset({0, 6, 14}, "number of supports");
  int dim = 3;
  ...
  Kokkos::parallel_for(
    "inside team", tp.set_scratch_size(1, Kokkos::PerTeam(1000)),
    KOKKOS_LAMBDA(const pcms::member_type& team) {
      int league_rank = team.league_rank();

      int num_supports = nsupports[league_rank];
      pcms::ScratchMatView local_supports(team.team_scratch(1), num_supports,
                                          dim);
      ...
      Kokkos::parallel_for(
        Kokkos::TeamThreadRange(team, num_supports), [=](int i) {
          for (int j = 0; j < dim; ++j) {
            int index = nsupports_offset[league_rank] * dim + i * dim + j;

            all_target_coordinates[index] = target_coordinates[league_rank][j];
            if (league_rank == 0) {
